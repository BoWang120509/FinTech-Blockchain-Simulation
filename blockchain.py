...
####################################################################
    def proof_of_work(self, block):
        ...
        #这个东西是挖矿本体。
        #根据难度设定，要求hash的开头有几个0。
        #如果不满足，nonce加1，再次计算hash，重复重复再重复。具体重复次数可以从main的output中看。
        #至于为什么这么做，Lec3也讲过，要求攻击者必须修改历史区块，回头重新做PoW并且再次追上honest chain才能做到覆写，这几乎不可能。
...
####################################################################   
    ...
    def mine_block(self, miner_address="Miner", max_tx_per_block=5):
        ...
        print(f"Block mined, Nonce = {new_block.nonce}")
        print(f"Hash = {proof}")
